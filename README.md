...
## 可观测、可运维与开放能力（本 Fork 新增）

在 OpenList 可靠性之外，本 Fork 还补齐了"配了但不知道哪一步断了"这一类问题：

| 能力 | 入口 | 说明 |
| --- | --- | --- |
| **系统自检** | 工具 → 系统自检 | 一行一件事的检查清单（配置目录 / 下载路径 / 下载器登录 / 磁盘 / TMDB / BGM / Mikan / 通知 / 任务线程），每项给出结论 + 证据 + 下一步建议 |
| **下载历史** | 历史 | 成功侧对账：近 N 天完成/失败/成功率/体积 + 每日趋势；支持按订阅与结果筛选 |
| **媒体库** | 媒体库 | 全局库视图（番剧数 / 视频数 / 占用空间）+ 搜索 + 单订阅剧集明细 |
| **手动补种** | 工具 → 手动补种 | 聚合「主 RSS + 备用 RSS + 任意粘贴的 RSS」搜索条目，标注来源与本地状态后直接下单 |
| **磁盘监控** | 设置 → 基本 → 其他 | 周期探测下载/迁移/配置目录空间，按档位去重推送预警（网络盘判为「不可测」而非 0%） |
| **追番周报** | 设置 → 基本 → 其他 | 汇总本周新增/失败/漏集，可选有漏集时自动补种 |
| **事件 Webhook** | 设置 → 基本 → 其他 | 发送**结构化 JSON 事件**（9 种）而非渲染文本，供 HomeAssistant / n8n / 自建看板消费 |
| **订阅分享** | 订阅页 | 生成分享码（**白名单复制**，不含本地路径/进度/评分/密钥），一键导入 |
| **只读令牌** | 设置 → 登录 | 用只读令牌访问只能「看和播」，写操作一律 403，适合分享给家人 |
| **订阅优先级** | 编辑订阅 → 优先级 | 高/普通/低三档；RSS 一轮扫描按优先级**稳定排序**，本季在追的先扫 |
| **RSS 并发度** | 设置 → 基本 → 其他 | 默认 3（保持既有语义），上限 8 |
| **字幕匹配** | 字幕匹配 | 字幕统一手动管理：**手动上传本地字幕**或**从射手网(ASSRT)获取**，写入前弹窗展示「改名前 / 改名后 / 对应的视频」二次确认；原子写，覆盖同名文件前自动备份到视频同目录的 `sub_bak/`（不存在则新建）。下载完成后不再自动抓取 |
| **AI 助手（MCP）** | MCP 客户端 | 新增写操作与诊断工具：启停订阅 / 刷新订阅 / 取消任务 / 精确重下 / **订阅诊断**（给出根因与下一步建议） |
| **质量择优规则** | 设置 → 基本 → 质量择优 | 可选分辨率/编码/体积/字幕组/做种数规则；默认关闭，规则过严自动回退，不会让某集永远不下 |

### 质量择优规则

- 全局设置：**设置 → 基本设置 → 质量择优**；单个订阅：**编辑订阅 → 自定义 → 质量择优**
- 支持分辨率顺序与上下限、编码偏好/排除、体积上下限、字幕组偏好/排除、做种数下限、合集优先
- **默认关闭**，关闭时沿用原有「分辨率 + 体积」逻辑
- 优先级固定为：多字幕组共存 → 洗版主 RSS 优先 → 质量规则
- 如果规则太严把某一集的候选全部过滤，会自动回退为不过滤；RSS 没有做种数时不会误判为 0
