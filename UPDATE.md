...
`RenameCacheUtil` 名称条目的既有约定是「rename 成功后 remove」（Aria2 / Transmission），v3.4.25 引入的偏移条目漏了对称清理，且偏移 0（绝大多数订阅）也每次提交写一行。

- 偏移为 0 时不再写 `rssOffset:` 条目——rename 侧条目缺省即回退 0，行为完全一致，避免每个下载白写一行 SQLite
- 偏移非 0 的条目在 rename 成功后随名称条目一并清理（Aria2 `remove` 处补一行；qB 两个成功出口清理），失败保留供下轮重试
...
一个订阅可以挂多条备用 RSS，每条有自己独立的集数偏移（同一部番，Baha 源按累计集数编号 96，主源按季内集数编号 24）。此前 RSS 解析按**来源源**的偏移算出 `reName`（S04E24），但离线下载的「期望文件计划」与启发式归位都按**订阅**的偏移换算文件名集数——备用源偏移与订阅不同时，归位目标名与 `reName` 差出一个偏移量：**实测 S04E24 离线完成后被重命名成 S04E96**。单文件种子还会让 `filterEpisodes` 的期望集数与计划集数对不上（多视频时整份计划被清空，退回启发式后同样算错）。

- 条目随行携带来源偏移：`Item.rssOffset`，`ItemsUtil` 主/备两分支都打标
- 计划构建统一走 `planAni(ani, item)`：偏移与订阅一致时零开销，不一致时用副本按来源偏移重建（本地种子解析与磁力链后台解析两条路径同一口径）
- 启发式归位（`buildEpisodeRenameMap` / `collectionEpisodeReName`）同样按来源偏移平移文件名集数
- 条目未携带来源偏移（老数据 / 非 RSS 入口）时回退 0，保持改造前行为

已知边界：按集数匹配的兜底链（`hasEpisodeVideos` / 超时快照覆盖判定）暂未接入来源偏移，仅在「期望计划不可用（磁力元数据拿不到）」时可能受影响；计划路径按字节数匹配，不受影响。

#### 验证

后端 `mvn test` 全量 **976 通过 + 4 e2e / 0 失败 / 0 跳过**（相对 3.4.22 的 974 新增 2 项：`OpenListPlanSourceOffsetTest` 2）；反向验证（摘掉修复）定向用例必现 `expected: <24.0> but was: <96.0>`。
