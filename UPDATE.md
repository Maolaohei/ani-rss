...
- **毛玻璃 sticky 顶栏**：首页列表由内层滚动改为页面级滚动，`#header` 吸顶 + 78% 页底色毛玻璃（blur 14px/saturate 180%），滚动后加深至 90% 并加投影与边界线（亮暗双态）；老浏览器回退纯色、reduced-transparency 关闭模糊；滚动监听 passive + 卸载清理
